...
int main(void)
{
	...
	stat = xTaskCreate(ledlink, "ledblink", 200/4, 0, 4, &t);
	stat = xTaskCreate(st7735_demo, "demo", 1024/4, 0, 3, &t);
	//	stat = xTaskCreate(spitest, "spitest", 200/4, 0, 3, &t);
//	stat = xTaskCreate(basic_main, "basic_main", 1024/4, 0, 3, &t);
...
void spitest()
{
	...
	while (1)
	{
		unsigned char Send_Data[]={1,0x55,0xaa};//{0x81,0xff,0x00,0x81};//{'H','e','l','l'};//
		unsigned char rx_data[30];
		vTaskDelay(500);
 ...
 *
 */
void SysTick_Handler_baremetal (void)             // SysTick Interrupt Handler
{
	if(timingdelay != 0x00){
...
/*Hint: for register modification, user might use next macros declared in XMC4800.h
*
